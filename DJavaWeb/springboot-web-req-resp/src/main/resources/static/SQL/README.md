...
黑马 JavaWeb 课程配套：MySQL 数据库与表结构（DDL）练习脚本，放在 `springboot-web-req-resp` 静态资源下便于随课程查阅。
...
| `mysql_02.sql` | 表操作（DDL）：创建表与约束、查看、修改、删除；含 `tb_emp` 员工表示例 |
...
2. 先 `USE` 到目标库，再打开 `mysql_02.sql`，练习建表、约束、`ALTER`、`DROP`，并理解 `tb_emp` 字段含义
3. 需要查类型长度与取值范围时，对照 `MySQL数据类型.xlsx`

注意：

- `CREATE` / `DROP` 会改动本机数据，练习库建议使用独立库名（如 `db01`），勿在生产库执行
- `mysql_02.sql` 中部分语句依赖已存在的表（如 `tb_emp`）；若报错，先执行文末完整 `CREATE TABLE tb_emp` 再建改表练习
...
| 阶段 | 说明 |
|------|------|
| 当前 | 纯 SQL / DDL 入门，为后续 JDBC、MyBatis 做准备 |
